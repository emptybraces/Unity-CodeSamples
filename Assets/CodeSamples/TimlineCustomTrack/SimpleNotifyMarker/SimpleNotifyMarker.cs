...
namespace Emptybraces.Timeline
{
	...
	public class SimpleNotifyMarker : Marker, INotification, INotificationOptionProvider
	{
		...
		[Header("Target")]
		[NoFoldOut] public ExposedGameObject[] Receivers;
		[Serializable] public class ExposedReferenceHolder<T> where T : UnityEngine.Object { public ExposedReference<T> ExposedReference; }
		[Serializable] public sealed class ExposedGameObject : ExposedReferenceHolder<GameObject> { }
		...
		void OnValidate()
		{
			...
			{
				if (hash.Contains(i.ExposedReference.exposedName))
				{
					i.ExposedReference.exposedName = new PropertyName();
				}
				hash.Add(i.ExposedReference.exposedName);
			}
		}
#endif
	}
